...
linuscommits = git_log[git_log['author']=='Linus Torvalds']
first_commit_timestamp = linuscommits['timestamp'].min()

# determining the last sensible commit timestamp
last_commit_timestamp = pd.to_datetime('today')

# filtering out wrong timestamps
filtername = (git_log['timestamp'] >= first_commit_timestamp) & (git_log['timestamp'] < last_commit_timestamp)
corrected_log = git_log.loc[filtername]
...
#corrected_log.set_index('timestamp')
commits_per_year = corrected_log['timestamp'].groupby(corrected_log.timestamp.dt.year).agg('count')
...
# plot the data
commits_per_year.plot(kind='bar',title = 'Number of Commits to Linux Repository by Year',legend = None)
